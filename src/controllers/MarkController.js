...
module.exports.markLabVivaStudents = async(req,res) =>{
    const dept_id = req.params.dept_id;
    const session = req.params.session;
    const subject_id = req.params.subject_id;
    const teacher_id = req.params.teacher_id;
...
module.exports.markLabVivaEditStudents = async(req,res) =>{
    const dept_id = req.params.dept_id;
    const session = req.params.session;
    const subject_id = req.params.subject_id;
    const teacher_id = req.params.teacher_id;
...
module.exports.markSubjects = async(req, res) =>{
    ...
    if(exam_id != ""){
        try {
            const response = await Subject.find({exam_id, subject_type}).select({_id:1,subject_code:1,subject_mark:1}).sort({subject_code:'ascending'});
            return res.status(200).json({ response });
        } catch (error) {
            return res.status(500).json({errors: [{msg: error.message}]});
        }
...
module.exports.markAddLabViva = async(req,res)=>{
    const form = formidable();
    form.parse(req, async(err, fields, files) =>{
        const {state: {exam_id,session,semister}, marKArr} = fields;
        const errors = [];
...
module.exports.markUpdateLabViva = async(req,res)=>{
    const form = formidable();
    form.parse(req, async(err, fields, files) =>{
        const {state: {exam_id,session,semister}, marKArr} = fields;
        const errors = [];
            ...
            try {
                for (let i = 0; i < marKArr.length; i++) {
                    ...
                    if(check_entry[0].marks[i].student_id == marKArr[i].student_id){
                        ...
                        await Mark.updateOne({exam_id},
                            {$set:{'marks.$[elem].first_mark':marKArr[i].mark, 'marks.$[elem].final_mark':marKArr[i].mark}},
                            {arrayFilters: [{'elem.subject_id':marKArr[i].subject_id,'elem.student_id':marKArr[i].student_id}]})
                    }
                 }
...
module.exports.markUpdate = async(req, res) =>{
    ...
    form.parse(req, async(err, fields, files) =>{
        const {state: {exam_id,session,semister}, marKArr, examinarType} = fields;
        //Check entry for First and Second Examinar
        let check_entry;
        if(examinarType == "first_examinar" || examinarType == "second_examinar"){
             check_entry = await Mark.aggregate([ 
                {$match:{exam_id: new mongoose.Types.ObjectId(exam_id)}},
                {$project: {
                    "exam_id":1,
                    marks: {
        ...
        if(errors.length !== 0){
            ...
            if(examinarType == "first_examinar"){
                ...
                //Check Entry for third examinar
                const check_entry = await Mark.aggregate([ 
                    {$match:{exam_id: new mongoose.Types.ObjectId(exam_id)}},
                    {$project: {
                        "exam_id":1,
